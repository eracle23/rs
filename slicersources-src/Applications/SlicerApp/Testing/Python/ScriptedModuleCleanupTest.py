...
def check_exit_code(slicer_executable, testing_enabled=True, debug=False):
    ...
    try:
        ...
        copyfile(currentDirPath + "/ScriptedModuleCleanupTestHelperModule.py", temporaryModuleDirPath + "/ModuleCleanup.py")

        common_arguments = [
            "--no-splash",
            "--disable-builtin-modules",
            "--additional-module-path",
            temporaryModuleDirPath,
            "--python-code",
            'slicer.util.selectModule("ModuleCleanup")',
        ]
