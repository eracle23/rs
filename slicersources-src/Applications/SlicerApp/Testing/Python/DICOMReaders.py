...
class DICOMReadersTest(ScriptedLoadableModuleTest):
    ...
    def test_AlternateReaders(self):
        ...
        for dataset in referenceData:
            try:
                ...
                dicomFilesDirectory = SampleData.downloadFromURL(fileNames=dataset["fileName"], loadFileTypes="ZipFile", uris=dataset["url"], checksums=dataset["checksum"])[0]
    ...
    def test_MissingSlices(self):
        ...
        dicomFilesDirectory = SampleData.downloadFromURL(fileNames="deidentifiedMRHead-dcm-one-series.zip", loadFileTypes="ZipFile", uris=TESTING_DATA_URL + "SHA256/899f3f8617ca53bad7dca0b2908478319e708b48ff41dfa64b6bac1d76529928", checksums="SHA256:899f3f8617ca53bad7dca0b2908478319e708b48ff41dfa64b6bac1d76529928")[0]
        self.delayDisplay("Finished with download\n")

        seriesUID = "1.3.6.1.4.1.5962.99.1.3814087073.479799962.1489872804257.270.0"
        seriesRASBounds = [-87.29489517211913, 81.70450973510744, -121.57139587402344, 134.42860412597656, -138.71430206298828, 117.28569793701172]
        seriesDirectory = "Series 004 [MR - SAG RF FAST VOL FLIP 20]"
        lastSliceCorners = [[[81.05451202, 133.92860413, 116.78569794], [81.05451202, -122.07139587, 116.78569794]], [[81.05451202, 133.92860413, -139.21429443], [81.05451202, -122.07139587, -139.21429443]]]
        filesToRemove = [
