...
def checkUserSettings(slicer_executable, common_args, keep_temporary_settings=False):
    ...
    args.extend(["--python-code", 'slicer.app.userSettings().setValue("foo", "bar"); print("foo: %s" % slicer.app.userSettings().value("foo"))'])
    ...
    args.extend(["--python-code", 'if slicer.app.userSettings().value("foo") ' + condition + ': raise Exception("' + error + '.")'])
    assert runSlicerAndExit(slicer_executable, args)[0] == EXIT_SUCCESS
    print("=> ok\n")
...
def checkRevisionUserSettings(slicer_executable, common_args, keep_temporary_settings=False):
    ...
    args = list(common_args)
    args.extend(["--python-code", 'slicer.app.revisionUserSettings().setValue("foo", "bar"); print("foo: %s" % slicer.app.revisionUserSettings().value("foo"))'])
    assert runSlicerAndExit(slicer_executable, args)[0] == EXIT_SUCCESS
    print("=> ok\n")
    ...
    if keep_temporary_settings:
        ...
        condition = '!= "bar"'
        error = "Setting foo should be set to bar"
    args.extend(["--python-code", 'if slicer.app.revisionUserSettings().value("foo") ' + condition + ': raise Exception("' + error + '.")'])
    assert runSlicerAndExit(slicer_executable, args)[0] == EXIT_SUCCESS
    print("=> ok\n")
