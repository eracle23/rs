...
def test_nrrd_dwi_load(first_file, second_file=None):
    ...
    for i in range(0, slicer_numgrads):
        ...
        np.testing.assert_almost_equal(bval_parsed, dw_node.GetBValue(i), decimal=7, err_msg="MRMLNode b value does not match NRRD header")
    ...
    for i in range(0, slicer_numgrads):
        ...
        np.testing.assert_array_almost_equal(parsed_gradient, attr_gradient, decimal=12, err_msg="NHDR gradient does not match gradient in node attribute dictionary")
