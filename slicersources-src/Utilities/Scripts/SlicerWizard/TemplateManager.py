...
class TemplateManager:
    ...
    def _copyAndReplace(self, inFile, template, destination, key, name):
        ...
        if isinstance(name, bytes):
            ...
            if encoding is not None:
                if confidence < 0.5:
                    logging.warning("%s: encoding detection confidence is %f:" " copied file might be corrupt" % (p, confidence))
    ...
    def addArguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="append",
            help="add additional template path for specified" " template category; if no category, expect that" " PATH contains subdirectories for one or more" " possible categories",
        )
        parser.add_argument(
            "--templateKey",
            metavar="TYPE=KEY",
            ...
            for tp in args.templatePath:
                ...
                if len(tpParts) == 1:
                    ...
                    if tpParts[0].lower() not in _templateCategories:
                        die(("'%s' is not a recognized template category" % tpParts[0], "recognized categories: %s" % ", ".join(_templateCategories)))
