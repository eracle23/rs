...
class ExtensionWizard:
    ...
    def _setExtensionUrl(self, project, name, value):
        ...
        if confirm:
            logging.info("Your extension currently uses '%s' for %s," " which can be changed to '%s' to point to your new" " public repository." % (oldValue, name, value))
    ...
    def publish(self, args):
        ...
        if r is None:
            ...
            if branch.name != "main":
                logging.warning("You are currently on the '%s' branch. " "It is strongly recommended to publish" " the 'main' branch." % branch)
        ...
        try:
            ...
            if createdRepo:
                ...
                r.index.commit("ENH: Update extension information\n\n" "Set %s information to reference" " new github repository." % name)
    ...
    def contribute(self, args):
        ...
        try:
            ...
            if xiRepo is None:
                ...
                if xiRemote is None:
                    raise Exception("the extension index repository ('%s')" " is not a clone of %s" % (xiRepo.working_tree_dir, forkedRepo.clone_url))
            ...
            if pullRequest is None:
                ...
                pullRequest = upstreamRepo.create_pull(title=msg[0], body="\n".join(msg[1:]), head=pull, base=args.target)
    ...
    def _execute(self, args):
        ...
        parser.add_argument(
            "--create",
            metavar="<TYPE:>NAME",
            help="create TYPE extension NAME" " under the destination directory;" " any modules are added to the new extension" " (default type: 'default')",
        )
        parser.add_argument(
            "--addModule",
            metavar="TYPE:NAME",
            action="append",
            help="add new TYPE module NAME to an existing project" " in the destination directory;" " may use more than once",
        )
        ...
        parser.add_argument(
            "--describe",
            action="store_true",
            help="print the extension description (json) to standard output",
        )
