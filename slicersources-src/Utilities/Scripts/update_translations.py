...
def update_translations(component, source_code_dir, translations_dir, lupdate_path, language=None, remove_obsolete_strings=False, source_file_regex=None, keep_temporary_files=False):
    if language is None:
...
def main(argv):
    ...
    parser.add_argument("--lupdate", default="lupdate", dest="lupdate_path", help="location of the Qt lupdate executable. Qt-6.3 is required for Python translation")
    parser.add_argument("-f", "--source-filter", dest="source_filter_regex", default=None, help="regular expression that filters file paths (file is included if the path matches the expression)")
    parser.add_argument("-s", "--source-code", metavar="DIR", dest="source_code_dir", help="folder of application source code (root of repository, such as https://github.com/Slicer/Slicer)", required=True)
    parser.add_argument("-c", "--component", dest="component", default="Slicer", help="translation component name (Slicer, CTK, ...)")
    parser.add_argument("-t", "--translations", dest="translations_dir", metavar="DIR", default="-", help="folder containing .ts translation files that will be updated" " (translations folder of https://github.com/Slicer/SlicerLanguageTranslations)")
    parser.add_argument("-l", "--language", default=None, dest="language", help="choose specific ts file to update by language (e.g., use en-US to update only US English translation file)")
    parser.add_argument("-r", "--remove-obsolete-strings", default=False, dest="remove_obsolete_strings", action="store_true", help="removes obsolete source strings (by calling lupdate with -noobsolete argument)")
    parser.add_argument("-v", "--verbose", default=False, dest="verbose", action="store_true", help="show more progress information")
    parser.add_argument("-k", "--keep-temporary-files", default=False, dest="keep_temporary_files", action="store_true", help="keep temporary files (for debugging the translation process)")
    ...
    if args.component == "Slicer":
        ...
        _generate_translation_header_from_json_file(os.path.join(args.source_code_dir, "Modules/Loadable/Volumes/Resources/VolumeDisplayPresets.json"), ["name", "description"])
    else:
        extract_translatable_from_cli_modules([args.source_code_dir])

    update_translations(args.component, args.source_code_dir, args.translations_dir, args.lupdate_path, args.language, args.remove_obsolete_strings, args.source_filter_regex, args.keep_temporary_files)
