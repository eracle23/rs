...
def getLongFlagDefinition(currentNode):
    ...
    if labelNodeList.length > 0:
        labelNode = labelNodeList[0]  # Only get the first one
        return "{}{}{}".format('[<span style="color:orange">--', getTextValuesFromNode(labelNode.childNodes), "</span>]")
    return ""
...
def getLabelDefinition(currentNode):
    ...
    if labelNodeList.length > 0:
        labelNode = labelNodeList[0]  # Only get the first one
        return "{}{}{}".format("** <span style=\"color:green\">'''", getTextValuesFromNode(labelNode.childNodes), "'''</span>")
    return ""
...
def DumpSEMMediaWikiFeatures(executableNode):
    ...
    for parameterNode in executableNode.getElementsByTagName("parameters"):
        outRegion += "* <span style=\"color:blue\">'''''{}'''''</span>: {}\n".format(getThisNodesInfoAsText(parameterNode, "label"), getThisNodesInfoAsText(parameterNode, "description"))
            ...
            if currentNode.nodeType == currentNode.ELEMENT_NODE:
                ...
                if getThisNodesInfoAsText(currentNode, "label") != "":
                    ...
                    if getThisNodesInfoAsText(currentNode, "default") != "":
                        outRegion += "{} {} {}: {} {}\n".format(getLabelDefinition(currentNode), getLongFlagDefinition(currentNode), getFlagDefinition(currentNode), getThisNodesInfoAsText(currentNode, "description"), getDefaultValueDefinition(currentNode))
                    else:
                        outRegion += "{} {} {}: {}\n\n".format(getLabelDefinition(currentNode), getLongFlagDefinition(currentNode), getFlagDefinition(currentNode), getThisNodesInfoAsText(currentNode, "description"))
