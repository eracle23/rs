...
class DICOMScalarVolumePluginClass(DICOMPlugin):
    ...
    def settingsPanelEntry(panel, parent):
        ...
        readersComboBox.toolTip = _("Preferred back end.  Archetype was used by default in Slicer before June of 2017." " Change this setting if data that previously loaded stops working (and report an issue).")
        formLayout.addRow(_("DICOM reader approach:"), readersComboBox)
        panel.registerProperty("DICOM/ScalarVolume/ReaderApproach", readersComboBox, "currentIndex", str(qt.SIGNAL("currentIndexChanged(int)")))
        ...
        panel.registerProperty("DICOM/ScalarVolume/AcquisitionGeometryRegularization", importFormatsComboBox, "currentUserDataAsString", str(qt.SIGNAL("currentIndexChanged(int)")), _("DICOM examination settings"), ctk.ctkSettingsPanel.OptionRequireRestart)
        ...
        allowLoadingByTimeCheckBox.toolTip = _("Offer loading of individual slices or group of slices" " that were acquired at a specific time (content or trigger time)." " If this option is enabled then a large number of loadable items may be" " displayed in the Advanced section of DICOM browser.")
        ...
        panel.registerProperty("DICOM/ScalarVolume/AllowLoadingByTime", allowLoadingByTimeMapper, "valueAsInt", str(qt.SIGNAL("valueAsIntChanged(int)")), _("DICOM examination settings"), ctk.ctkSettingsPanel.OptionRequireRestart)
        # DICOM examination settings are cached so we need to restart to make sure changes take effect
    ...
    def compareVolumeNodes(volumeNode1, volumeNode2):
        ...
        if image1.GetScalarType() != image2.GetScalarType():
            comparison += _("First volume is {imageScalarType1}, but second is {imageScalarType2}").format(imageScalarType1=image1.GetScalarTypeAsString(), imageScalarType2=image2.GetScalarTypeAsString())
        ...
        for tag in subseriesTags:
            if len(subseriesValues[tag]) > 1:
                ...
                for valueIndex, value in enumerate(subseriesValues[tag]):
                    ...
                    loadable.tooltip = _("{count} files, grouped by {tag} = {value}. First file: {filename}").format(count=len(loadable.files), tag=tag, value=value, filename=loadable.files[0])
    ...
    def load(self, loadable, readerApproach=None):
        ...
        if volumeNode:
            ...
            self.acquisitionModeling.createAcquisitionTransform(volumeNode, addAcquisitionTransformIfNeeded=self.acquisitionGeometryRegularizationEnabled(), hardenAcquisitionTransform=self.hardenAcquisitionGeometryRegularization())
    ...
    class AcquisitionModeling:
        ...
        def gridTransformFromCorners(self, volumeNode, sourceCorners, targetCorners):
            ...
            if not (sourceCorners.shape == cornerShape and targetCorners.shape == cornerShape):
                raise Exception("Corner shapes do not match volume dimensions %s, %s, %s" % (sourceCorners.shape, targetCorners.shape, cornerShape))
        ...
        def createAcquisitionTransform(self, volumeNode, addAcquisitionTransformIfNeeded=True, hardenAcquisitionTransform=False):
            ...
            if maxError > self.cornerEpsilon:
                ...
                logging.debug("Irregular volume geometry detected, but maximum error is within tolerance" + f" (maximum error of {maxError:g} mm, tolerance threshold is {self.cornerEpsilon:g} mm).")
