        ...
        for filePath in files:
            ...
            if canBeCineMri and sopClassUID == "1.2.840.10008.5.1.4.1.1.4":  # MR Image Storage
                ...
                if not (instanceNumber in instanceNumberToLoadableIndex.keys()):
                    ...
                    if modality == "US":
                        ...
                        if hasattr(ds, "SequenceOfUltrasoundRegions"):
                            if len(ds.SequenceOfUltrasoundRegions) == 1:
                                ...
                                TISSUE = 1
                                COLOR_FLOW = 2
                                if hasattr(region, "RegionSpatialFormat") and region.RegionSpatialFormat == TISSUE_2D and hasattr(region, "RegionDataType") and (region.RegionDataType == TISSUE or region.RegionDataType == COLOR_FLOW) and hasattr(region, "PhysicalDeltaX") and hasattr(region, "PhysicalDeltaY") and hasattr(region, "PhysicalUnitsXDirection") and region.PhysicalUnitsXDirection == UNITS_CM and hasattr(region, "PhysicalUnitsYDirection") and region.PhysicalUnitsYDirection == UNITS_CM:
                    ...
                    loadable.tooltip = _("{modality} image sequence ({count} planes)").format(modality=modality, count=len(loadables[loadableIndex].files))
    ...
    def load(self, loadable):
        ...
        for fileIndex, filePath in enumerate(loadable.files):
            ...
            if loadable.singleSequence:
                ...
                outputSequenceNode, playbackRateFps = self.addSequenceFromImageData(imageData, tempFrameVolume, filePath, loadable.name, (len(loadable.files) == 1), spacingMmPerPixel)
                outputSequenceNodes.append(outputSequenceNode)
