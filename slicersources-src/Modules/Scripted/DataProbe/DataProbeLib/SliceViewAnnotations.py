...
class SliceAnnotations(VTKObservationMixin):
    ...
    def onBackgroundLayerPersistenceCheckBox(self):
        ...
        settings = qt.QSettings()
        settings.setValue("DataProbe/sliceViewAnnotations.bgDICOMAnnotationsPersistence", self.backgroundDICOMAnnotationsPersistence)
        self.updateSliceViewFromGUI()
        ...
        if self.sliceViews[sliceViewName]:
            ...
            if backgroundVolume is not None and foregroundVolume is not None:
                if self.bottomLeft:
                    ...
                    self.cornerTexts[0]["2-Foreground"]["text"] = "F: " + foregroundVolumeName + " (" + str("%d" % (foregroundOpacity * 100)) + "%)"
            ...
            if self.topRight and sliceNode.GetSlabReconstructionEnabled():
                ...
                self.cornerTexts[3]["8-SlabReconstructionThickness"]["text"] = "Thickness: " + str(sliceNode.GetSlabReconstructionThickness()) + " " + unitNode.GetSuffix()
                self.cornerTexts[3]["9-SlabReconstructionType"]["text"] = "Type: " + sliceNode.GetSlabReconstructionTypeAsString(sliceNode.GetSlabReconstructionType())
            else:
            ...
            if (labelVolume is not None) and self.bottomLeft:
                ...
                self.cornerTexts[0]["1-Label"]["text"] = "L: " + labelVolumeName + " (" + str("%d" % (labelOpacity * 100)) + "%)"
    ...
    def makeDicomAnnotation(self, bgUid, fgUid, sliceViewName):
        ...
        if fgUid is not None and bgUid is not None:
            ...
            if self.topLeft and viewHeight > 150:
                if backgroundDicomDic["Patient Name"] != foregroundDicomDic["Patient Name"] or backgroundDicomDic["Patient ID"] != foregroundDicomDic["Patient ID"] or backgroundDicomDic["Patient Birth Date"] != foregroundDicomDic["Patient Birth Date"]:
                    for key in self.cornerTexts[2]:
                        self.cornerTexts[2][key]["text"] = ""
                else:
