...
class EndoscopyWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def updateWidgetFromMRML(self, *_unused):
        ...
        self.saveOrientationButton.text = _("Update Keyframe Orientation") if deletable else _("Save Keyframe Orientation")
    ...
    @staticmethod
    def _allViewNodeIDs():
        return [slicer.mrmlScene.GetNthNodeByClass(nodeIndex, "vtkMRMLViewNode").GetID() for nodeIndex in range(slicer.mrmlScene.GetNumberOfNodesByClass("vtkMRMLViewNode"))] + [slicer.mrmlScene.GetNthNodeByClass(nodeIndex, "vtkMRMLSliceNode").GetID() for nodeIndex in range(slicer.mrmlScene.GetNumberOfNodesByClass("vtkMRMLSliceNode"))]
...
class EndoscopyLogic:
    ...
    * Orientation 3x3 matrices serve as an intermediate representation for conversion between camera position,
    ...
    .. code-block:: python
        ...
        EndoscopyWidget._hideOnlyInView(
            EndoscopyWidget._viewNodeIDFromCameraNode(cameraNode), [inputCurve]
        )
        ...
        def flyToNext():
            ...
            resampledCurvePointIndex = (
                resampledCurvePointIndex + 1
            ) % logic.getNumberOfControlPoints()
    ...
    def setControlPointsByResamplingAndInterpolationFromInputCurve(self, inputCurve) -> None:
        ...
        if self.inputCurve.GetNumberOfControlPoints() > 1:
            ...
            pointsPerSegment = int(
                (self.inputCurve.GetCurveLengthWorld() / (self.dl / 8.0)) / (self.inputCurve.GetNumberOfControlPoints() - 1) + 1,
            )
    ...
    def updateCameraFromOrientationAtIndex(self, resampledCurvePointIndex):
        ...
        if resampledCurve is None or not 0 <= resampledCurvePointIndex < resampledCurve.GetNumberOfControlPoints():
            return
        ...
        adjustedFocalPoint = cameraPosition + worldMatrix3x3[:, 2] * (np.linalg.norm(focalPoint - cameraPosition) / np.linalg.norm(worldMatrix3x3[:, 2]))
    ...
    def getDefaultOrientation(curve, curveControlPointIndex, planeNormal):
        ...
        lastPoint = curveControlPointIndex == numberOfCurveControlPoints - 1 and not curve.GetCurveClosed()
        if lastPoint:
    ...
    def indexOfControlPointForDistanceAlongCurve(curve, distanceAlongInputCurve):
        indexOfControlPoint = curve.GetCurvePointIndexAlongCurveWorld(0, distanceAlongInputCurve) // curve.GetNumberOfPointsPerInterpolatingSegment()
    ...
    def getCameraOrientationsFromInputCurve(inputCurve):
        ...
        convertedCameraOrientations = dict() if cameraOrientationsString is None else json.loads(cameraOrientationsString)
    ...
    def cameraOrientationsIsCorrectType(convertedCameraOrientations):
        return (
            ...
            and all(
                [isinstance(v, list) and len(v) == 4 and all([isinstance(e, (int, float)) for e in v]) for v in convertedCameraOrientations.values()],
            )
        )
