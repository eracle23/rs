...
class VectorToScalarVolume(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.contributors = ["Steve Pieper (Isomics)", "Pablo Hernandez-Cerdan (Kitware)", "Jean-Christophe Fillion-Robin (Kitware)"]
...
class VectorToScalarVolumeWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def updateGUIFromParameterNode(self, caller=None, event=None):
        ...
        self.ui.methodSelectorComboBox.setCurrentIndex(self.ui.methodSelectorComboBox.findData(self._parameterNode.ConversionMethod))
    ...
    def isValidInputOutputData(inputVolumeNode, outputVolumeNode, conversionMethod, componentToExtract):
        ...
        if conversionMethod is ConversionMethods.SINGLE_COMPONENT:
            ...
            if not 0 <= componentToExtract < numberOfComponents:
                msg = _("component to extract ({componentSelected}) is invalid. Image has only {componentsTotal} components.").format(componentSelected=componentToExtract, componentsTotal=numberOfComponents)
                logging.debug("isValidInputOutputData failed: %s" % msg)
                return False, msg
        ...
        if conversionMethod is ConversionMethods.LUMINANCE:
            if numberOfComponents < 3:
                msg = _("input has only {componentsTotal} components but requires " "at least 3 components for luminance conversion.").format(componentsTotal=numberOfComponents)
                logging.debug("isValidInputOutputData failed: %s" % msg)
                return False, msg
