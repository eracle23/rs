...
class SlicerDICOMBrowser(VTKObservationMixin, qt.QWidget):
    ...
    def getLoadablesFromFileLists(self, fileLists):
        ...
        if missingFileCount > 0:
            messages.append(_("{missing_file_count} of {total_file_count} selected files listed in the database cannot be found on disk.").format(missing_file_count=missingFileCount, total_file_count=allFileCount))
        ...
        if missingFileCount < allFileCount:
            ...
            loadablesByPlugin, loadEnabled = DICOMLib.getLoadablesFromFileLists(fileLists, self.enabledPluginClassNames, messages, lambda progressLabel, progressValue, progressDialog=progressDialog: progressCallback(progressDialog, progressLabel, progressValue), self.pluginInstances)

            progressDialog.setParent(None)
            progressDialog.close()

        if messages:
            slicer.util.warningDisplay(_("Warning: {messages}").format(messages=" ".join(messages)) + "\n\n" + _("See python console for error message."), windowTitle=_("DICOM"), parent=self)
    ...
    def proceedWithReferencedLoadablesSelection(self):
        ...
        messages = []
        loadedNodeIDs = DICOMLib.loadLoadables(self.loadablesByPlugin, messages, lambda progressLabel, progressValue, progressDialog=progressDialog: progressCallback(progressDialog, progressLabel, progressValue))
        ...
        for plugin in self.loadablesByPlugin:
            for loadable in self.loadablesByPlugin[plugin]:
                if loadable.selected and loadable.warning != "":
                    ...
                    logging.warning(_("Warning in DICOM plugin {load_type} when examining loadable {name}: {message}").format(load_type=plugin.loadType, name=loadable.name, message=loadable.warning))
...
class DICOMReferencesDialog(qt.QMessageBox):
    ...
    WINDOW_TEXT = _("The loaded DICOM objects contain references to other datasets you did not select for loading." " Please select Yes if you would like to load the following referenced datasets, No if you only want to load the" " originally selected series, or Cancel to abort loading.")
