    ...
    if expectedSelectedPlugins is not None and len(expectedSelectedPlugins.keys()) > 0:
        ...
        for pluginName in expectedSelectedPlugins.keys():
            if pluginName not in actualSelectedPlugins:
                ...
                logging.error("DICOM plugin '%s' was expected to be selected in %d loadables, but was selected in %d" % (pluginName, expectedSelectedPlugins[pluginName], actualSelectedPlugins[pluginName]))
                success = False
    ...
    if expectedLoadedNodes is not None:
        for nodeType in expectedLoadedNodes.keys():
            ...
            if numOfLoadedNodes != expectedLoadedNodes[nodeType]:
                logging.error("Number of loaded %s nodes was %d, but %d was expected" % (nodeType, numOfLoadedNodes, expectedLoadedNodes[nodeType]))
                success = False
...
class LoadDICOMFilesToDatabase:
    ...
    def __init__(self, url, archiveFilePath=None, dicomDataDir=None, expectedNumberOfFiles=None, selectedPlugins=None, loadedNodes=None, checksum=None):
        from time import gmtime, strftime
    ...
    def __enter__(self):
        if slicer.util.downloadAndExtractArchive(self.url, self.archiveFilePath, self.dicomDataDir, self.expectedNumberOfExtractedFiles, checksum=self.checksum):
        ...
        for n in range(1, len(files)):
            ...
            if abs(spaceError) > epsilon:
                ...
                if acquisitionGeometryRegularizationEnabled:
                    ...
                    warningText += "  If loaded image appears distorted, enable 'Acquisition geometry regularization'" " in Application settings / DICOM / DICOMScalarVolumePlugin. Please use caution.\n"
                break
...
def loadLoadables(loadablesByPlugin, messages=None, progressCallback=None):
    ...
    for step, (loadable, plugin) in enumerate(selectedLoadables.items(), start=1):
        ...
        try:
            ...
            logging.error("DICOM plugin failed to load '" + loadable.name + "' as a '" + plugin.loadType + "'.\n" + traceback.format_exc())
...
def importFromDICOMWeb(
    ...
    try:
        ...
        for seriesIndex, currentSeriesInstanceUID in enumerate(seriesInstanceUIDs):
            ...
            try:
                seriesInfo = client.retrieve_series_metadata(study_instance_uid=studyInstanceUID, series_instance_uid=currentSeriesInstanceUID)
                ...
                if bulkRetrieve:
                    instances = client.iter_series(study_instance_uid=studyInstanceUID, series_instance_uid=currentSeriesInstanceUID)
