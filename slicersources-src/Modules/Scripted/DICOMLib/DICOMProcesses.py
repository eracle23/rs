...
class DICOMStoreSCPProcess(DICOMProcess):
    ...
    def onStateChanged(self, newState):
        ...
        if stderr and stderr.size():
            slicer.util.errorDisplay("An error occurred. For further information click 'Show Details...'", windowTitle=self.__class__.__name__, detailedText=str(stderr))
    ...
    def start(self, cmd=None, args=None):
        ...
        onReceptionCallback = '%s --load-short --print-short --print-filename --search PatientName "%s/#f"' % (self.dcmdumpExecutable, self.incomingDataDir)
        args = [str(self.port), "--accept-all", "--output-directory", self.incomingDataDir, "--exec-sync", "--exec-on-reception", onReceptionCallback]
