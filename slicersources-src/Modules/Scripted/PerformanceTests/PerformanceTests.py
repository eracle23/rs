...
class PerformanceTestsWidget(ScriptedLoadableModuleWidget):
    ...
    def reslicing(self, iters=100):
        ...
        result = "%d x %d, fps = %.1f (%.1f +/- %.2f ms per frame) - see details in table '%s'" % (dims[0], dims[1], 1.0 / renderingTimeMean, 1000.0 * renderingTimeMean, 1000.0 * renderingTimeStd, resultTableNode.GetName())
        print(result)
        self.log.insertHtml("<i>%s</i>" % result)
        self.log.insertPlainText("\n")
        self.log.ensureCursorVisible()
