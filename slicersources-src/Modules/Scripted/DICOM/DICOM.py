...
class DICOM(ScriptedLoadableModule):
    def __init__(self, parent):
        ...
        self.parent.helpText = _("This module allows importing, loading, and exporting DICOM files," " and sending receiving data using DICOM networking.")
    ...
    def performPostModuleDiscoveryTasks(self):
        ...
        if not slicer.app.commandOptions().noMainWindow:
            ...
            layout = '<layout type="horizontal">' " <item>" "  <dicombrowser></dicombrowser>" " </item>" "</layout>"
            layoutNode = slicer.app.layoutManager().layoutLogic().GetLayoutNode()
            layoutNode.AddLayoutDescription(slicer.vtkMRMLLayoutNode.SlicerLayoutDicomBrowserView, layout)
    ...
    def onURLReceived(self, urlString):
        ...
        importedSeriesInstanceUIDs = DICOMUtils.importFromDICOMWeb(dicomWebEndpoint=queryMap["dicomweb_endpoint"], studyInstanceUID=queryMap["studyUID"], accessToken=accessToken, bulkRetrieve=bulkRetrieve)
    ...
    def initializeDICOMDatabase(self):
        ...
        if slicer.app.commandOptions().testingEnabled:
            ...
            databaseDirectory = os.path.join(slicer.app.temporaryPath, "temp" + slicer.app.applicationName + "DICOMDatabase_" + ctk.ctkDICOMDatabase().schemaVersion())
        else:
            # For production
    ...
    def onLayoutChanged(self, viewArrangement):
        ...
        if self.currentViewArrangement != slicer.vtkMRMLLayoutNode.SlicerLayoutNone and self.currentViewArrangement != slicer.vtkMRMLLayoutNode.SlicerLayoutDicomBrowserView:
    ...
    def onBrowserWidgetClosed(self):
        if self.currentViewArrangement != slicer.vtkMRMLLayoutNode.SlicerLayoutDicomBrowserView and self.currentViewArrangement != slicer.vtkMRMLLayoutNode.SlicerLayoutNone:
        ...
        if layoutId == slicer.vtkMRMLLayoutNode.SlicerLayoutNone or layoutId == slicer.vtkMRMLLayoutNode.SlicerLayoutDicomBrowserView:
...
class _ui_DICOMSettingsPanel:
    def __init__(self, parent):
        ...
        parent.registerProperty(slicer.dicomDatabaseDirectorySettingsKey, directoryButton, "directory", str(qt.SIGNAL("directoryChanged(QString)")), _("DICOM general settings"), ctk.ctkSettingsPanel.OptionRequireRestart)
        ...
        loadReferencesComboBox.toolTip = _("Determines whether referenced DICOM series are" " offered when loading DICOM, or the automatic behavior if interaction is disabled." " Interactive selection of referenced series is the default selection")
        ...
        parent.registerProperty("DICOM/automaticallyLoadReferences", loadReferencesComboBox, "currentUserDataAsString", str(qt.SIGNAL("currentIndexChanged(int)")))

        detailedLoggingCheckBox = qt.QCheckBox()
        detailedLoggingCheckBox.toolTip = _("Log more details during DICOM operations." " Useful for investigating DICOM loading issues but may impact performance.")
        ...
        parent.registerProperty("DICOM/detailedLogging", detailedLoggingMapper, "valueAsInt", str(qt.SIGNAL("valueAsIntChanged(int)")))
        detailedLoggingCheckBox.stateChanged.connect(self.onDetailedLoggingStateChanged)

        thumbnailsSizeComboBox = ctk.ctkComboBox()
        thumbnailsSizeComboBox.toolTip = _("Determines the relative size of the thumbnails when using the visual DICOM browser")
        ...
        parent.registerProperty("DICOM/thumbnailsSize", thumbnailsSizeComboBox, "currentUserDataAsString", str(qt.SIGNAL("currentIndexChanged(int)")), _("DICOM settings"), ctk.ctkSettingsPanel.OptionRequireRestart)
...
class DICOMFileDialog:
    ...
    def createDefaultDatabase():
        ...
        slicer.util.warningDisplay(_("Could not create a DICOM database with default settings. Please create a new database or" " update the existing incompatible database using options shown in DICOM browser."))
        return False
    ...
    def dropEvent(self):
        ...
        if not DICOMFileDialog.validDirectories(self.directoriesToAdd):
            message = _("Import of files that have special (non-ASCII) characters in their names is not supported." " It is recommended to move files into a different folder and retry." " Try to import from current location anyway?")
...
class DICOMWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        self.copyOnImportAction.setToolTip(_("If enabled, all imported files are copied into the DICOM database." " This is useful when importing from removable drives."))
    ...
    def onClearDatabase(self):
        ...
        if len(patientIds) == 0:
            ...
            message = _("Are you sure you want to delete all data and files copied into the database ({count} patients)?").format(count=len(patientIds))
