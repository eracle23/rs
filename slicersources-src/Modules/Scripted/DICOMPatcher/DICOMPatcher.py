...
class DICOMPatcherWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        self.normalizeFileNamesCheckBox.setToolTip(_("Replace file and folder names with automatically generated names." " Fixes errors caused by file path containins special characters or being too long."))
        ...
        self.forceSamePatientNameIdInEachDirectoryCheckBox.setToolTip(_("Generate patient name and ID from the first file in a directory and force all" " other files in the same directory to have the same patient name and ID." " Enable this option if a separate patient directory is created for each patched file."))
        ...
        self.forceSameSeriesInstanceUidInEachDirectoryCheckBox.setToolTip(_("Generate a new series instance UID for each directory" " and set it in all files in that same directory." " Enable this option to force placing all frames in a folder into a single volume."))
        ...
        self.generateMissingIdsCheckBox.setToolTip(_("Generate missing patient, study, series IDs. It is assumed that" " all files in a directory belong to the same series. Fixes error caused by too aggressive anonymization" " or incorrect DICOM image converters."))
        ...
        self.generateImagePositionFromSliceThicknessCheckBox.setToolTip(_("Generate 'image position sequence' for multi-frame files that only have" " 'SliceThickness' field. Fixes error in Dolphin 3D CBCT scanners."))
        ...
        self.fixExposureFiascoCheckBox.setToolTip("If checked, then some invalid exposure tags are converted to valid tags" " (see the 'exposure fiasco' - http://dclunie.blogspot.com/2008/11/dicom-exposure-attribute-fiasco.html).")
        ...
        self.splitUltrasoundSeriesByInstanceNumberCheckBox.setToolTip(_("If checked, then ultrasound image series are split by instance number." " Useful if many ultrasound acquisitions appear in the same series."))
        ...
        self.specifyCharacterSetCheckBox.setToolTip("If checked, then SpecificCharacterSet is set to the specified value in all DICOM files" " where the character set is not UTF8 (ISO_IR 192) already. It is recommended to enable this if patient name does not show up correctly.")
        ...
        self.anonymizeDicomCheckBox.setToolTip(_("If checked, then some patient identifiable information will be removed from the patched DICOM files." " There are many fields that can identify a patient, this function does not remove all of them."))
