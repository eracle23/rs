...
class ClosedSurfaceSegmentStatisticsPlugin(SegmentStatisticsPluginBase):
    ...
    def computeStatistics(self, segmentID):
        ...
        containsClosedSurfaceRepresentation = segmentationNode.GetSegmentation().ContainsRepresentation(vtkSegmentationCore.vtkSegmentationConverter.GetSegmentationClosedSurfaceRepresentationName())
    ...
    def getMeasurementInfo(self, key):
        ...
        #
        if key == "surface_mm2":
            return self.createMeasurementInfo(name="Surface mm2", title=_("Surface area"), description=_("Surface area computed from closed surface representation."), units=_("mm2"), quantityDicomCode=self.createCodedEntry("000247", "99CHEMINF", "Surface area", True), unitsDicomCode=self.createCodedEntry("mm2", "UCUM", "square millimeter", True))

        elif key == "volume_mm3":
            return self.createMeasurementInfo(name="Volume mm3", title=_("Volume"), description=_("Volume computed from closed surface representation."), units=_("mm3"), quantityDicomCode=self.createCodedEntry("118565006", "SCT", "Volume", True), unitsDicomCode=self.createCodedEntry("mm3", "UCUM", "cubic millimeter", True))

        elif key == "volume_cm3":
            return self.createMeasurementInfo(name="Volume cm3", title=_("Volume"), description=_("Volume computed from closed surface representation."), units=_("cm3"), quantityDicomCode=self.createCodedEntry("118565006", "SCT", "Volume", True), unitsDicomCode=self.createCodedEntry("cm3", "UCUM", "cubic centimeter", True))
