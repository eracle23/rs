...
class SegmentStatisticsWidget(ScriptedLoadableModuleWidget):
    ...
    def onNodeSelectionChanged(self):
        self.applyButton.enabled = self.segmentationSelector.currentNode() is not None and self.parameterNodeSelector.currentNode() is not None
...
class SegmentStatisticsLogic(ScriptedLoadableModuleLogic):
    ...
    registeredPlugins = [LabelmapSegmentStatisticsPlugin, ScalarVolumeSegmentStatisticsPlugin, ClosedSurfaceSegmentStatisticsPlugin]
    ...
    def registerPlugin(plugin):
        ...
        for key in plugin.keys:
            if key.count(".") > 0:
                logging.warning("Plugin keys should not contain extra '.' as it might mix pluginname.measurementkey in " "the parameter node")
    ...
    def exportToTable(self, table, nonEmptyKeysOnly=True):
        ...
        for key in keys:
            ...
            measurements = [statistics[segmentID, key] for segmentID in statistics["SegmentIDs"] if (segmentID, key) in statistics]
...
class SegmentStatisticsTest(ScriptedLoadableModuleTest):
    ...
    def test_SegmentStatisticsPlugins(self):
        ...
        self.assertEqual(columnHeaders, ["Segment", "Volume mm3 (LM)", "Voxel count", "Volume mm3 (SV)", "Volume cm3 (SV)", "Minimum", "Maximum", "Mean", "Standard deviation", "Percentile 5", "Percentile 95", "Median", "Surface mm2", "Volume mm3 (CS)", "Volume cm3 (CS)"])
