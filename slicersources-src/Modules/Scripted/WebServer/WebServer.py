...
class WebServerWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        self.enableCORSCheck.toolTip = _("Enable Cross Origin Resource Sharing (stop server to change option)." " Warning: this option allows any web page to access the server.")
        ...
        self.enableSlicerHandlerExec.toolTip = _("Enable execution of arbitrary Python command using Slicer API." " It only has effect if Slicer API is enabled, too (stop server to change option).")
    ...
    def startServer(self):
        ...
        self.logic.addDefaultRequestHandlers(enableSlicer=self.enableSlicerHandler.checked, enableExec=self.enableSlicerHandlerExec.checked, enableDICOM=self.enableDICOMHandler.checked, enableStaticPages=self.enableStaticPagesHandler.checked)
...
class SlicerHTTPServer(HTTPServer):
    ...
    def __init__(self, server_address: tuple[str, int] = ("", 2016), requestHandlers: list[BaseRequestHandler] = None, docroot: str = ".", logMessage: Callable = None, certfile: str = None, keyfile: str = None, enableCORS: bool = False):
        """
    ...
    class SlicerRequestCommunicator:
        ...
        def __init__(self, connectionSocket: socket.socket, requestHandlers: list[BaseRequestHandler], docroot: str, logMessage: BaseRequestLoggingFunction, enableCORS: bool):
            """
        ...
        def onWritable(self, fileno):
            ...
            try:
                ...
                self.sentSoFar += sent
                self.logMessage("sent: %d (%d of %d, %f%%)" % (sent, self.sentSoFar, self.toSend, 100.0 * self.sentSoFar / self.toSend))
            except OSError as e:
...
class WebServerLogic:
    ...
    def __init__(self, port: Optional[int] = None, enableSlicer: bool = True, enableExec: bool = False, enableDICOM: bool = True, enableStaticPages: bool = True, requestHandlers: list[BaseRequestHandler] = None, logMessage: Optional[BaseRequestLoggingFunction] = None, enableCORS: bool = False):
    ...
    def start(self):
        ...
        certfile = None
        keyfile = None
        self.server = SlicerHTTPServer(requestHandlers=self.requestHandlers, docroot=self.docroot, server_address=("", self.port), logMessage=self.logMessage, certfile=certfile, keyfile=keyfile, enableCORS=self.enableCORS)
        self.server.start()
