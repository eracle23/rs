...
class _ui_SettingsPanel:
    ...
    def __init__(self, parent):
        ...
        self.addRow(_("Additional template paths:"), self.genericPaths)

        self.paths = {}

        for category in SlicerWizard.TemplateManager.categories():
            self.paths[category] = DirectoryListWidget()
            self.addRow(_("Additional template paths for {category}:").format(category=category), self.paths[category])
...
class SettingsPanel(ctk.ctkSettingsPanel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.registerProperty(userTemplatePathKey(), self.ui.genericPaths.ui.pathList, "directoryList", str(qt.SIGNAL("directoryListChanged()")), "Additional template paths", ctk.ctkSettingsPanel.OptionRequireRestart)

        for category in self.ui.paths.keys():
            self.registerProperty(userTemplatePathKey(category), self.ui.paths[category].ui.pathList, "directoryList", str(qt.SIGNAL("directoryListChanged()")), "Additional template paths for %s" % category, ctk.ctkSettingsPanel.OptionRequireRestart)
