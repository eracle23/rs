...
class CreateComponentDialog:
    ...
    def accept(self):
        if not len(self.componentName):
            slicer.util.errorDisplay(_("{type} name may not be empty.").format(type=self._typetc), windowTitle=_("Cannot create {type}").format(type=self._typelc), parent=self.dialog)
            return

        if self.showDestination:
            dest = self.destination
            if not len(dest) or not os.path.exists(dest):
                slicer.util.errorDisplay(_("Destination must be an existing directory."), windowTitle=_("Cannot create {type}").format(type=self._typelc), parent=self.dialog)
                return
