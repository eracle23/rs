...
class ExtensionWizardWidget:
            ...
            try:
                ...
                if repo is None:
                    ...
                    if os.path.exists(destination):
                        raise OSError("create extension: refusing to overwrite" " existing directory '%s'" % destination)
                    ...
                    if os.path.exists(cmakeFile):
                        raise OSError("create extension: refusing to overwrite" " directory containing CMakeLists.txt file at '%s'" % dlg.destination)

                path = self.templateManager.copyTemplate(destination, "extensions", dlg.componentType, dlg.componentName, createInSubdirectory, requireEmptyDirectory)

            except:
                if not slicer.util.confirmRetryCloseDisplay(_("An error occurred while trying to create the extension."), parent=self.parent.window(), detailedText=traceback.format_exc()):
                    return
    ...
    def selectExtension(self, path=None):
        if path is None or isinstance(path, bool):
            path = qt.QFileDialog.getExistingDirectory(self.parent.window(), _("Select Extension..."), self.extensionLocation)
        ...
        try:
            ...
            slicer.util.errorDisplay(_("Failed to open extension {path}.").format(path=path), parent=self.parent.window(), detailedText=traceback.format_exc(), standardButtons=qt.QMessageBox.Close)
            return False
        ...
        if len(candidates):
            ...
            if dlg.exec_() == qt.QDialog.Accepted:
                ...
                if len(failed):
                    ...
                    slicer.util.errorDisplay(text, parent=parent, windowTitle=_("Module loading failed"), standardButtons=qt.QMessageBox.Close, informativeText=detailedInformation)

                    return

                # Instantiate and load requested module(s)
                if not factory.loadModules([module.key for module in modulesToLoad]):
                    text = _("The module factory manager reported an error. " "One or more of the requested module(s) and/or " "dependencies thereof may not have been loaded.")
                    slicer.util.errorDisplay(text, parent=parent, windowTitle=_("Error loading module(s)"), standardButtons=qt.QMessageBox.Close)
    ...
    def createExtensionModule(self):
        ...
        while dlg.exec_() == qt.QDialog.Accepted:
            ...
            try:
                ...
                if not slicer.util.confirmRetryCloseDisplay(_("An error occurred while trying to create the module."), parent=self.parent.window(), detailedText=traceback.format_exc()):
                    return
            ...
            try:
                ...
                detailedInformation = _("The module has been created, but the extension" " CMakeLists.txt could not be updated. In order" " to include the module in the extension build," " you will need to update the extension" " CMakeLists.txt by hand.")
                slicer.util.errorDisplay(text, parent=self.parent.window(), detailedText=traceback.format_exc(), standardButtons=qt.QMessageBox.Close, informativeText=detailedInformation)
