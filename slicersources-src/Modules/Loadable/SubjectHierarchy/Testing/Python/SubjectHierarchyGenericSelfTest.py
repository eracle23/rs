...
class SubjectHierarchyGenericSelfTestTest(ScriptedLoadableModuleTest):
    ...
    def section_LoadDicomDataWitchBatchProcessing(self):
        try:
            ...
            with DICOMUtils.TemporaryDICOMDatabase(self.dicomDatabaseDir) as db:
                ...
                with DICOMUtils.LoadDICOMFilesToDatabase(self.dicomZipFileUrl, self.dicomZipFilePath, self.dicomDataDir, self.expectedNumOfFilesInDicomDataDir, {}, loadedNodes, checksum=self.dicomZipChecksum) as success:
    ...
    def section_CLI(self):
        ...
        # Resample
        resampleParameters = {"outputPixelSpacing": "24.5,24.5,11.5", "interpolationType": "lanczos", "InputVolume": ctVolumeNode.GetID(), "OutputVolume": resampledVolumeNode.GetID()}
    ...
    def section_AttributeFilters(self):
        ...
        sceneFile = SampleData.downloadFromURL(fileNames=self.attributeFilterTestSceneFileName, uris=self.attributeFilterTestSceneFileUrl, loadFileTypes="SceneFile", checksums=self.attributeFilterTestSceneChecksum)[0]
