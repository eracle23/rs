...
""")
    ...
    def setupOptionsFrame(self):
        ...
        self.softEdgeMmSpinBox.setToolTip(_("Standard deviation of the Gaussian function that blurs the edge of the mask." " Higher value makes the edge softer."))
        ...
        for button in self.operationRadioButtons:
            button.connect("toggled(bool)", lambda toggle, widget=self.buttonToOperationNameMap[button]: self.onOperationSelectionChanged(widget, toggle))
    ...
    def updateGUIFromMRML(self):
        ...
        self.softEdgeMmSpinBox.setValue(float(self.scriptedEffect.parameter("SoftEdgeMm")) if self.scriptedEffect.parameter("SoftEdgeMm") else 0)
    ...
    def onApply(self):
        with slicer.util.tryWithErrorDisplay(_("Failed to apply mask to volume."), waitCursor=True):
            ...
            SegmentEditorMaskVolumeEffect.maskVolumeWithSegment(segmentationNode, segmentID, operationMode, fillValues, inputVolume, outputVolume, softEdgeMm=softEdgeMm)
