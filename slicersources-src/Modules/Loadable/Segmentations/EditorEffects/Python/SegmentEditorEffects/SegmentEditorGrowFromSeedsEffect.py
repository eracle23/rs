...
class SegmentEditorGrowFromSeedsEffect(AbstractScriptedSegmentEditorAutoCompleteEffect):
    ...
    def setupOptionsFrame(self):
        ...
        self.seedLocalityFactorSlider.setToolTip(_("Increasing this value makes the effect of seeds more localized," " thereby reducing leaks, but requires seed regions to be more evenly distributed in the image." ' The value is specified as an additional "intensity level difference" per "unit distance."'))
    ...
    def computePreviewLabelmap(self, mergedImage, outputLabelmap):
        ...
        logging.info("Grow-cut operation on volume of {}x{}x{} voxels was completed in {:3.1f} seconds.".format(self.clippedMasterImageData.GetDimensions()[0], self.clippedMasterImageData.GetDimensions()[1], self.clippedMasterImageData.GetDimensions()[2], time.time() - startTime))
