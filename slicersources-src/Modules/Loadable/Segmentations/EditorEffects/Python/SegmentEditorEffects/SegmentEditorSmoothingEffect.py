...
If segments overlap, segment higher in the segments table will have priority. <b>Applied to all visible segments.</b>
    ...
    def setupOptionsFrame(self):
        ...
        self.kernelSizeMMSpinBox.setToolTip(_("Diameter of the neighborhood that will be considered around each voxel. " "Higher value makes smoothing stronger (more details are suppressed)."))
        ...
        self.gaussianStandardDeviationMMSpinBox.setToolTip(_("Standard deviation of the Gaussian smoothing filter coefficients. " "Higher value makes smoothing stronger (more details are suppressed)."))
        ...
        self.applyToAllVisibleSegmentsCheckBox.setToolTip(_("Apply smoothing effect to all visible segments in this segmentation node. This operation may take a while."))
        self.applyToAllVisibleSegmentsCheckBox.objectName = self.__class__.__name__ + "ApplyToAllVisibleSegments"
        self.applyToAllVisibleSegmentsLabel = self.scriptedEffect.addLabeledOptionsWidget(_("Apply to visible segments:"), self.applyToAllVisibleSegmentsCheckBox)
    ...
    def onApply(self, maskImage=None, maskExtent=None):
        ...
        applyToAllVisibleSegments = int(self.scriptedEffect.parameter("ApplyToAllVisibleSegments")) != 0 if self.scriptedEffect.parameter("ApplyToAllVisibleSegments") else False
        ...
        try:
            ...
            if smoothingMethod == JOINT_TAUBIN:
                ...
                for index in range(inputSegmentIDs.GetNumberOfValues()):
                    ...
                    self.showStatusMessage(_("Smoothing {segmentName}...").format(segmentName=segmentationNode.GetSegmentation().GetSegment(segmentID).GetName()))
    ...
    def clipImage(self, inputImage, maskExtent, margin):
        ...
        clipper.SetOutputWholeExtent(maskExtent[0] - margin[0], maskExtent[1] + margin[0], maskExtent[2] - margin[1], maskExtent[3] + margin[1], maskExtent[4] - margin[2], maskExtent[5] + margin[2])
    ...
    def modifySelectedSegmentByLabelmap(self, smoothedImage, selectedSegmentLabelmap, modifierLabelmap, maskImage, maskExtent):
        if maskImage:
            ...
            slicer.vtkOrientedImageDataResample.ModifyImage(maskImage, selectedSegmentLabelmap, slicer.vtkOrientedImageDataResample.OPERATION_MAXIMUM)
            slicer.vtkOrientedImageDataResample.ModifyImage(maskImage, smoothedClippedSelectedSegmentLabelmap, slicer.vtkOrientedImageDataResample.OPERATION_MINIMUM)
            ...
            self.scriptedEffect.modifySelectedSegmentByLabelmap(maskImage, slicer.qSlicerSegmentEditorAbstractEffect.ModificationModeSet, updateExtent)
        else:
    ...
    def smoothMultipleSegments(self, maskImage=None, maskExtent=None):
        ...
        if not segmentationNode.GenerateMergedLabelmapForAllSegments(mergedImage, vtkSegmentationCore.vtkSegmentation.EXTENT_UNION_OF_SEGMENTS_PADDED, None, visibleSegmentIds):
        ...
        for segmentId, labelValue in segmentLabelValues:
            ...
            self.scriptedEffect.modifySegmentByLabelmap(segmentationNode, segmentId, smoothedBinaryLabelMap, slicer.qSlicerSegmentEditorAbstractEffect.ModificationModeSet, False)
