...
<li><b>Apply:</b> set the previewed segmentation in the selected segment. Previous contents of the segment is overwritten.
    ...
    def setupOptionsFrame(self):
        ...
        self.autoThresholdModeSelectorComboBox.setToolTip(_("How to set lower and upper values of the threshold range." " Threshold above/below: sets the range from the computed value to maximum/minimum." " Set as lower/upper value: only modifies one side of the threshold range."))
        ...
        self.selectPreviousAutoThresholdButton.setToolTip(_("Select previous thresholding method and set thresholds." " Useful for iterating through all available methods."))

        self.selectNextAutoThresholdButton = qt.QToolButton()
        self.selectNextAutoThresholdButton.text = ">"
        self.selectNextAutoThresholdButton.setToolTip(_("Select next thresholding method and set thresholds." " Useful for iterating through all available methods."))
    ...
    def onSelectPreviousAutoThresholdMethod(self):
        self.autoThresholdMethodSelectorComboBox.currentIndex = (self.autoThresholdMethodSelectorComboBox.currentIndex - 1) % self.autoThresholdMethodSelectorComboBox.count
    ...
    def updatePreviewDisplayPipelines(self):
        ...
        for sliceViewName in sliceViewNames:
            ...
            segmentationDisplayableManager = sliceWidget.sliceView().displayableManagerByClassName("vtkMRMLSegmentationsDisplayableManager2D")
            existingSegmentRendererTag = segmentationDisplayableManager.GetCustomSegmentRendererTag(self.previewedSegmentationDisplayNode.GetID(), self.previewedSegmentID)
            ...
            pipeline.customRendererTag = segmentationDisplayableManager.AddCustomSegmentRenderer(self.previewedSegmentationDisplayNode.GetID(), self.previewedSegmentID)
...
class HistogramEventFilter(qt.QObject):
    ...
    def eventFilter(self, object, event):
        ...
        if event.type() == qt.QEvent.GraphicsSceneMousePress or event.type() == qt.QEvent.GraphicsSceneMouseMove or event.type() == qt.QEvent.GraphicsSceneMouseRelease:
