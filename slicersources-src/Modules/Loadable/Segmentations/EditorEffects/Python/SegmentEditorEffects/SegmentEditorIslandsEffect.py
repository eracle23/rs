...
about each operation, hover the mouse over the option and wait for the tooltip to appear.""")
    ...
    def setupOptionsFrame(self):
        ...
        self.keepLargestOptionRadioButton.setToolTip(_("Keep only the largest island in selected segment, remove all other islands in the segment."))
        ...
        self.keepSelectedOptionRadioButton.setToolTip(_("Click on an island in a slice view to keep that island and remove all other islands in selected segment."))
        ...
        self.removeSmallOptionRadioButton.setToolTip(_("Remove all islands from the selected segment that are smaller than the specified minimum size."))
        ...
        self.removeSelectedOptionRadioButton.setToolTip(_("Click on an island in a slice view to remove it from selected segment."))
        ...
        self.addSelectedOptionRadioButton.setToolTip(_("Click on a region in a slice view to add it to selected segment."))
        ...
        self.splitAllOptionRadioButton.setToolTip(_("Create a new segment for each island of selected segment. Islands smaller than minimum size will be removed. " "Segments will be ordered by island size."))
        ...
        for operationRadioButton in self.operationRadioButtons:
            operationRadioButton.connect("toggled(bool)", lambda toggle, widget=self.widgetToOperationNameMap[operationRadioButton]: self.onOperationSelectionChanged(widget, toggle))
    ...
    def splitSegments(self, minimumSize=0, maxNumberOfSegments=0, split=True):
        ...
        with slicer.util.NodeModify(segmentationNode):
            ...
            self.scriptedEffect.modifySegmentByLabelmap(segmentationNode, selectedSegmentID, emptyLabelmap, slicer.qSlicerSegmentEditorAbstractEffect.ModificationModeSet)
            ...
            for i in range(labelValues.GetNumberOfTuples()):
                ...
                if i != 0 and split:
                    ...
                    segment.AddRepresentation(slicer.vtkSegmentationConverter.GetSegmentationBinaryLabelmapRepresentationName(), selectedSegment.GetRepresentation(slicer.vtkSegmentationConverter.GetSegmentationBinaryLabelmapRepresentationName()))
    ...
    def processInteractionEvents(self, callerInteractor, eventId, viewWidget):
        ...
        if operationName == ADD_SELECTED_ISLAND:
            ...
            if not segmentationNode.GenerateMergedLabelmapForAllSegments(inputLabelImage, vtkSegmentationCore.vtkSegmentation.EXTENT_UNION_OF_SEGMENTS_PADDED, None, visibleSegmentIds):
