...
class AbstractScriptedSegmentEditorAutoCompleteEffect(AbstractScriptedSegmentEditorEffect):
    ...
    def onSegmentationModified(self, caller, event):
        ...
        for segmentIndex in range(self.selectedSegmentIds.GetNumberOfValues()):
            ...
            if segmentID in self.selectedSegmentModifiedTimes and segmentLabelmap and segmentLabelmap.GetMTime() == self.selectedSegmentModifiedTimes[segmentID]:
    ...
    def observeSegmentation(self, observationEnabled):
        ...
        if observationEnabled and segmentation is not None:
            ...
            observedEvents = [vtkSegmentationCore.vtkSegmentation.SegmentAdded, vtkSegmentationCore.vtkSegmentation.SegmentRemoved, vtkSegmentationCore.vtkSegmentation.SegmentModified, vtkSegmentationCore.vtkSegmentation.SourceRepresentationModified]
    ...
    def getPreviewNode(self):
        ...
        if previewNode and self.scriptedEffect.parameterDefined("SegmentationResultPreviewOwnerEffect") and self.scriptedEffect.parameter("SegmentationResultPreviewOwnerEffect") != self.scriptedEffect.name:
    ...
    def onApply(self):
        ...
        previewContainsClosedSurfaceRepresentation = previewNode.GetSegmentation().ContainsRepresentation(slicer.vtkSegmentationConverter.GetSegmentationClosedSurfaceRepresentationName())
        ...
        for index in range(segmentIDs.GetNumberOfValues()):
            ...
            self.scriptedEffect.modifySegmentByLabelmap(segmentationNode, segmentID, previewSegmentLabelmap, slicer.qSlicerSegmentEditorAbstractEffect.ModificationModeSet)
    ...
    def getPreviewShow3D(self):
        ...
        if not previewNode:
            return False
        containsClosedSurfaceRepresentation = previewNode.GetSegmentation().ContainsRepresentation(slicer.vtkSegmentationConverter.GetSegmentationClosedSurfaceRepresentationName())
    ...
    def effectiveExtentChanged(self):
        ...
        effectiveGeometryString = segmentationNode.GetSegmentation().DetermineCommonLabelmapGeometry(vtkSegmentationCore.vtkSegmentation.EXTENT_UNION_OF_EFFECTIVE_SEGMENTS, self.selectedSegmentIds)
        ...
        return (
            (masterImageExtent[0] != currentLabelExtent[0] and currentLabelExtent[0] > effectiveLabelExtent[0] - self.minimumExtentMargin)
            or (masterImageExtent[1] != currentLabelExtent[1] and currentLabelExtent[1] < effectiveLabelExtent[1] + self.minimumExtentMargin)
            or (masterImageExtent[2] != currentLabelExtent[2] and currentLabelExtent[2] > effectiveLabelExtent[2] - self.minimumExtentMargin)
            or (masterImageExtent[3] != currentLabelExtent[3] and currentLabelExtent[3] < effectiveLabelExtent[3] + self.minimumExtentMargin)
            or (masterImageExtent[4] != currentLabelExtent[4] and currentLabelExtent[4] > effectiveLabelExtent[4] - self.minimumExtentMargin)
            or (masterImageExtent[5] != currentLabelExtent[5] and currentLabelExtent[5] < effectiveLabelExtent[5] + self.minimumExtentMargin)
        )
    ...
    def preview(self):
        ...
        if self.effectiveExtentChanged():
            ...
            if self.minimumNumberOfSegments != self.minimumNumberOfSegmentsWithEditableArea:
                editableAreaSpecified = self.scriptedEffect.parameterSetNode().GetSourceVolumeIntensityMask() or self.scriptedEffect.parameterSetNode().GetMaskMode() != slicer.vtkMRMLSegmentationNode.EditAllowedEverywhere
                if editableAreaSpecified and self.selectedSegmentIds.GetNumberOfValues() < self.minimumNumberOfSegmentsWithEditableArea:
                    ...
                    raise RuntimeError(_("Minimum {minimumNumberOfSegments} visible segments are required.").format(minimumNumberOfSegments=self.minimumNumberOfSegmentsWithEditableArea))
                    ...
                    raise RuntimeError(_("Minimum {minimumNumberOfSegments} visible segments (or specification of editable area or intensity range) is required.").format(minimumNumberOfSegments=self.minimumNumberOfSegments))
                ...
                self.selectedSegmentIds = None
                raise RuntimeError(_("Minimum {minimumNumberOfSegments} visible segments are required.").format(minimumNumberOfSegments=self.minimumNumberOfSegments))
            ...
            commonGeometryString = segmentationNode.GetSegmentation().DetermineCommonLabelmapGeometry(vtkSegmentationCore.vtkSegmentation.EXTENT_UNION_OF_EFFECTIVE_SEGMENTS, self.selectedSegmentIds)
            ...
            margin = [int(max(3, self.extentGrowthRatio * (labelsEffectiveExtent[1] - labelsEffectiveExtent[0]))), int(max(3, self.extentGrowthRatio * (labelsEffectiveExtent[3] - labelsEffectiveExtent[2]))), int(max(3, self.extentGrowthRatio * (labelsEffectiveExtent[5] - labelsEffectiveExtent[4])))]
            labelsExpandedExtent = [max(masterImageExtent[0], labelsEffectiveExtent[0] - margin[0]), min(masterImageExtent[1], labelsEffectiveExtent[1] + margin[0]), max(masterImageExtent[2], labelsEffectiveExtent[2] - margin[1]), min(masterImageExtent[3], labelsEffectiveExtent[3] + margin[1]), max(masterImageExtent[4], labelsEffectiveExtent[4] - margin[2]), min(masterImageExtent[5], labelsEffectiveExtent[5] + margin[2])]
            ...
            previewNode.GetSegmentation().SetConversionParameter(slicer.vtkBinaryLabelmapToClosedSurfaceConversionRule.GetSmoothingFactorParameterName(), "-0.5")

            inputContainsClosedSurfaceRepresentation = segmentationNode.GetSegmentation().ContainsRepresentation(slicer.vtkSegmentationConverter.GetSegmentationClosedSurfaceRepresentationName())
        ...
        segmentationNode.GenerateMergedLabelmapForAllSegments(mergedImage, vtkSegmentationCore.vtkSegmentation.EXTENT_UNION_OF_EFFECTIVE_SEGMENTS, self.mergedLabelmapGeometryImage, self.selectedSegmentIds)
