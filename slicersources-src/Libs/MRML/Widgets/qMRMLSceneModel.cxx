  ...
  QStandardItem* item, vtkMRMLNode* node, int column)
{
  ...
  if (column == this->nameColumn())
  {
    QString nodeName = QString(node->GetName());
    QString tagName = QString(node->GetNodeTagName());
    QString displayName = nodeName;
    if (!tagName.isEmpty() && node->GetClassName())
    {
      if (nodeName == tagName)
      {
        QString translated = QCoreApplication::translate(node->GetClassName(), node->GetNodeTagName());
        if (translated != nodeName)
        {
          displayName = translated;
        }
      }
      else if (nodeName.startsWith(tagName + "_"))
      {
        QString translated = QCoreApplication::translate(node->GetClassName(), node->GetNodeTagName());
        if (translated != tagName)
        {
          displayName = translated + nodeName.mid(tagName.length());
        }
      }
    }
