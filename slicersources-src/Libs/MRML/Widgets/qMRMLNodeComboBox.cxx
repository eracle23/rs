...
void qMRMLNodeComboBoxPrivate::updateActionItems(bool resetRootIndex)
{
  ...
  if (q->mrmlScene())
  {
    ...
    if (this->AddEnabled)
    {
      ...
      {
        QString label = q->nodeTypeLabel(nodeType);
        extraItems.append(q->tr("Create new ") + label);
        if (this->RenameEnabled)
        {
          extraItems.append(q->tr("Create new ") + label + q->tr(" as..."));
        }
      }
    }
...
void qMRMLNodeComboBox::activateExtraItem(const QModelIndex& index)
{
  ...
  QString createNewStr = tr("Create new ");
  if (data.startsWith(createNewStr))
  {
    QString label = data.mid(createNewStr.length());
    QString nodeTypeName;
    QString asSuffixStr = tr(" as...");
    bool createAs = data.endsWith(asSuffixStr);
    for (const auto& nodeType : this->nodeTypes())
    {
      QString foundLabel = this->nodeTypeLabel(nodeType);
      if (createAs)
      {
        foundLabel += asSuffixStr;
      }
      if (foundLabel == label)
      {
...
void qMRMLNodeComboBox::renameCurrentNode()
{
  ...
  QString newName = getNameFromDialog(tr("Rename ") + this->nodeTypeLabel(node->GetClassName()), currentName, &ok);
...
void qMRMLNodeComboBox::createNodeAs(const QString& nodeTypeName)
{
    ...
    QString dialogTitle = tr("Create new ") + label;
    QString defaultName = "";
