...
for index in range(nodes.GetNumberOfItems()):
    ...
    table.append({"family": n.GetClassName().replace("vtkMRML", "").replace("Node", ""), "category": n.GetAttribute("Category"), "_type": n.GetTypeAsString(), "node_name": n.GetName(), "singleton_tag": n.GetSingletonTag(), "node_id": n.GetID()})

titles = {"family": "Family", "category": "Category", "_type": "Type", "node_name": "Node name", "singleton_tag": "Singleton Tag", "node_id": "Node ID"}
