...
To add a module, drag&drop it from the _Modules_ list above. Then use the advanced panel (>>) to reorganize/delete the modules within the toolbar.
...
This file is named `Slicer.ini` and it stores settings applying to _all versions_ of Slicer installed by the _current user_.
...
This file is named like `Slicer-<REVISION>.ini` and it stores settings applying to a _specific revision_ of Slicer installed by the _current user_.
