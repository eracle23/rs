...
Note that when code is pasted into an empty line then all the code in the clipboard is executed _at once_. If the current command line is not empty then the code from the clipboard is pasted into the console and executed _line by line_. When code is executed line by line, the behavior is different in that an empty input line immediately closes the current block, and output is printed after executing each line.
