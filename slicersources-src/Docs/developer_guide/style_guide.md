...
A good commit message title (first line) should **explain what the commit does for the user, not ''how'' it is done**. _How_ can be explained in the body of the commit message (if looking at the code of the commit is not self explanatory enough).

Examples:

- Bad: `BUG: Check pointer validity before dereferencing` -> _implementation detail_, _self-explanatory_ (by looking at the code)
...
- Bad: `COMP: Typo in cmake variable` -> _implementation detail_, _self-explanatory_
