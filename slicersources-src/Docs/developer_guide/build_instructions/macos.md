...
- While it is not enforced, we strongly recommend you to _avoid_ the use of _spaces_ for both the `source directory` and the `build directory`.
...
_Warning:_ An significant amount of disk space is required to compile Slicer in Debug mode (>20GB)

_Warning:_ Some firewalls will block the git protocol. See more information and solution [here](common_errors.md#firewall-is-blocking-git-protocol).
...
_Warning:_ Slicer will **only** create a valid package that will run on machines other than it's built on if Qt was built from source.
