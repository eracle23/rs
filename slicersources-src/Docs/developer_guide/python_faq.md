...
params = {
    "inputVolume": head.GetID(),
    "outputVolume": headLabel.GetID(),
    "seed": pointListNode.GetID(),
    "iterations": 2,
}

cliNode = slicer.cli.runSync(
    slicer.modules.simpleregiongrowingsegmentation, None, params
)
```
...
def onProcessingStatusUpdate(cliNode, event):
    ...
    if cliNode.GetStatus() & cliNode.Completed:
        if cliNode.GetStatus() & cliNode.ErrorsMask:
            ...
            # success
            print(
                "CLI execution succeeded. Output model node ID: "
                + cliNode.GetParameterAsString("OutputGeometry")
            )
...
Complete example: _How to emulate selection of `FOV, spacing match Volumes` checkbox in the slice view controller menu?_
