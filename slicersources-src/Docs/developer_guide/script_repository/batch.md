    ...
    for study in db.studiesForPatient(patient):
        ...
        for series in db.seriesForStudy(study):
            ...
            for instanceUID in db.instancesForSeries(series):
                qt.QFile.copy(
                    db.fileForInstance(instanceUID),
                    f"{temporaryDir.path()}/{instanceUID}.dcm",
                )
            patientID = slicer.dicomDatabase.instanceValue(instanceUID, "0010,0020")
            outputPath = os.path.join(
                convertedPath, patientID, study, series, "BatchResult"
            )
...
def segROI(segmentationNode):
    ...
    for segmentId in stats["SegmentIDs"]:
        ...
        roi.SetSize(
            obb_diameter_mm * 2
        )  # make the ROI twice the size of the segmentation
        ...
        boundingBoxToRasTransform = np.row_stack(
            (
                np.column_stack(((1, 0, 0), (0, 1, 0), (0, 0, 1), obb_center_ras)),
                (0, 0, 0, 1),
            )
        )
...
for labelFile in labelFiles:
    ...
    segLogic.ExportAllSegmentsToLabelmapNode(
        seg, labelmap, slicer.vtkSegmentation.EXTENT_REFERENCE_GEOMETRY
    )
