...
ScreenCapture.ScreenCaptureLogic().captureSliceSweep(
    getNode("vtkMRMLSliceNodeRed"), -125.0, 75.0, 30, "c:/tmp", "image_%05d.png"
)
```
...
for step in range(steps):
    offset = bounds[4] + step / (1.0 * steps) * (bounds[5] - bounds[4])
