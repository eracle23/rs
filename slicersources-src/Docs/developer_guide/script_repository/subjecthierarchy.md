...
shNode.CreateHierarchyItem(
    parentItemID, name, level
...
itemID = shNode.GetItemByUID(
    slicer.vtkMRMLSubjectHierarchyConstants.GetDICOMUIDName(), seriesInstanceUid
)
itemID = shNode.GetItemByUIDList(
    slicer.vtkMRMLSubjectHierarchyConstants.GetDICOMInstanceUIDName(), instanceUID
)
...
Displayed items can be filtered using _setAttributeFilter_ method. An example of the usage can be found in the [unit test](https://github.com/Slicer/Slicer/blob/53fb5b8acd41cb36eafbc5c4b66ff39c8434f4c6/Applications/SlicerApp/Testing/Python/SubjectHierarchyGenericSelfTest.py#L352-L360). Modified version here:
...
def exportNodes(shFolderItemId, outputFolder):
    ...
    for itemIdIndex in range(childIds.GetNumberOfIds()):
        ...
        if (
            dataNode
            and dataNode.IsA("vtkMRMLStorableNode")
            and dataNode.GetStorageNode()
        ):
