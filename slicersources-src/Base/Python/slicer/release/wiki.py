...
class Wiki:
    ...
    def update_version_info_pages(self, release_version):
        ...
        for page_short_name, page_name in Wiki.VERSION_INFO_PAGES.items():
            ...
            current_version = self.version_info(page_name)
            if current_version != updated_version:
                raise RuntimeError("Failed to update {}: {} version is {}".format(page_name, page_short_name, current_version))
    ...
    def update_redirect_pages(self, release_version):
        for redirect_page in Wiki.REDIRECT_PAGES:
            ...
            summary = "Update REDIRECT from Documentation/{} to Documentation/{}".format(current_version, release_version)
    ...
    def update_top_level_documentation_page(self, release_version):
        ...
        template = "* [[Documentation/{version}|{version}]] / " "[[Documentation/{version}/ReleaseNotes|Release notes]] / " "[[Documentation/{version}/Announcements | Announcement]] / " "[[Documentation/{version}/Acknowledgments | Acknowledgments]]"
        marker = "<!-- NEXT RELEASE -->"
        content = self.page_content(page_name)
...
def main():
    ...
    parser.add_argument(
        "--password",
        type=str,
        default=os.environ.get("SLICER_WIKI_UPDATEBOT_PWD"),
        help="password for 'UpdateBot' user. By default, try to get password from " "'SLICER_WIKI_UPDATEBOT_PWD' environment variable.",
    )

    subparsers = parser.add_subparsers(help="available sub-commands", dest="command")

    # sub-command parser
    parser_query = subparsers.add_parser("query", help="obtain version information")

    parser_query.add_argument(
        "--version-info",
        action="store_true",
        help="display the version associated with pages %s" % ", ".join(["%s" % page_name for page_name in Wiki.VERSION_INFO_PAGES.values()]),
    )
    parser_query.add_argument(
        "--next-version-info",
        action="store_true",
        help="display what would be the *next* version associated " "with pages %s" % ", ".join(["%s" % page_name for page_name in Wiki.VERSION_INFO_PAGES.values()]),
    )
    parser_query.add_argument(
        "--version-list",
        action="store_true",
        help="display the versions associated with page 'Template::Documentation/versionlist'",
    )
    parser_query.add_argument(
        "--acknowledgments-main-version",
        action="store_true",
        help="display the version associated with page 'Template:Documentation/acknowledgments-versionlist'",
    )
    parser_query.add_argument(
    ...
    # sub-command parser
    parser_copy = subparsers.add_parser("copy", help="copy Nightly pages into RELEASE_VERSION namespace")
    ...
    # sub-command parser
    parser_update = subparsers.add_parser("update", help="create and/or update wiki pages with RELEASE_VERSION")
    ...
    parser_update.add_argument(
        "--version-info-pages",
        action="store_true",
        help="update the version associated with pages %s" % ", ".join(["%s" % page_name for page_name in Wiki.VERSION_INFO_PAGES.values()]),
    )
    parser_update.add_argument(
        "--redirect-pages",
        action="store_true",
    ...
    parser_update.add_argument(
        "--version-list",
        action="store_true",
        help="add RELEASE_VERSION to page 'Template::Documentation/versionlist'",
    )
    parser_update.add_argument(
        "--acknowledgments-main-version",
        action="store_true",
        help="add RELEASE_VERSION to page 'Template:Documentation/acknowledgments-versionlist'",
    )
    parser_update.add_argument(
