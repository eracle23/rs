...
def setSliceViewerLayers(background="keep-current", foreground="keep-current", label="keep-current", foregroundOpacity=None, labelOpacity=None, fit=False, rotateToVolumePlane=False):
...
def loadFiberBundle(filename, returnNode=False):
    ...
    if app.ioManager().registeredFileReaderCount(readerType) == 0:
        ...
        errorMessage += f"\n\n{readerType} reader is implemented in the {readerClassName} class expected to be " f"registered by the {moduleName} module provided by the {extensionName} extension." "\n\nStatus:"
...
def _vtkArrayFromModelData(modelNode, arrayName, location):
    ...
    if not arrayVtk:
        ...
        raise ValueError("Input modelNode does not contain {} data array '{}'. Available array names: '{}'".format(location, arrayName, "', '".join(availableArrayNames)))
    return arrayVtk
...
def arrayFromMarkupsCurveData(markupsNode, arrayName, world=False):
    ...
    if not arrayVtk:
        ...
        raise ValueError("Input markupsNode does not contain curve point data array '{}'. Available array names: '{}'".format(arrayName, "', '".join(availableArrayNames)))
...
def dataframeFromMarkups(markupsNode):
    ...
    dataframe = pd.DataFrame({"label": label, "position.R": positionWorldR, "position.A": positionWorldA, "position.S": positionWorldS, "selected": selected, "visible": visible, "description": description})
    return dataframe
...
class VTKObservationMixin:
    ...
    def Observations(self):
        return [(obj, event, method, group, tag, priority) for obj, events in self.__observations.items() for event, methods in events.items() for method, (group, tag, priority) in methods.items()]
...
def infoDisplay(text, windowTitle=None, parent=None, standardButtons=None, **kwargs):
    ...
    _messageDisplay(logging.INFO, text, None, parent=parent, windowTitle=windowTitle, mainWindowNeeded=True, icon=qt.QMessageBox.Information, standardButtons=standardButtons, **kwargs)
...
def warningDisplay(text, windowTitle=None, parent=None, standardButtons=None, **kwargs):
    ...
    standardButtons = standardButtons if standardButtons else qt.QMessageBox.Ok
    _messageDisplay(logging.WARNING, text, None, parent=parent, windowTitle=windowTitle, mainWindowNeeded=True, icon=qt.QMessageBox.Warning, standardButtons=standardButtons, **kwargs)
...
def errorDisplay(text, windowTitle=None, parent=None, standardButtons=None, **kwargs):
    ...
    standardButtons = standardButtons if standardButtons else qt.QMessageBox.Ok
    _messageDisplay(logging.ERROR, text, None, parent=parent, windowTitle=windowTitle, mainWindowNeeded=True, icon=qt.QMessageBox.Critical, standardButtons=standardButtons, **kwargs)
...
def confirmOkCancelDisplay(text, windowTitle=None, parent=None, **kwargs):
    ...
    result = _messageDisplay(logging.INFO, text, True, parent=parent, windowTitle=windowTitle, icon=qt.QMessageBox.Question, standardButtons=qt.QMessageBox.Ok | qt.QMessageBox.Cancel, **kwargs)
...
def confirmYesNoDisplay(text, windowTitle=None, parent=None, **kwargs):
    ...
    if not windowTitle:
        windowTitle = slicer.app.applicationName + " confirmation"
    result = _messageDisplay(logging.INFO, text, True, parent=parent, windowTitle=windowTitle, icon=qt.QMessageBox.Question, standardButtons=qt.QMessageBox.Yes | qt.QMessageBox.No, **kwargs)
...
def confirmRetryCloseDisplay(text, windowTitle=None, parent=None, **kwargs):
    ...
    result = _messageDisplay(logging.ERROR, text, False, parent=parent, windowTitle=windowTitle, icon=qt.QMessageBox.Critical, standardButtons=qt.QMessageBox.Retry | qt.QMessageBox.Close, **kwargs)
...
def extractArchive(archiveFilePath, outputDir, expectedNumberOfExtractedFiles=None):
    ...
    if extractSuccessful is False or (expectedNumberOfExtractedFiles is not None and numOfFilesInOutputDirTest != expectedNumberOfExtractedFiles):
...
def downloadAndExtractArchive(url, archiveFilePath, outputDir, expectedNumberOfExtractedFiles=None, numberOfTrials=3, checksum=None):
...
def plot(narray, xColumnIndex=-1, columnNames=None, title=None, show=True, nodes=None):
    ...
    .. code-block:: python
      ...
      chartNode.GetNthPlotSeriesNode(0).SetPlotType(
