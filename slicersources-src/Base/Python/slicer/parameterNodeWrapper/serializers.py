...
class PathSerializer(Serializer):
    ...
    @staticmethod
    def canSerialize(type_) -> bool:
        return type_ in (pathlib.Path, pathlib.PosixPath, pathlib.WindowsPath, pathlib.PurePath, pathlib.PurePosixPath, pathlib.PureWindowsPath)
...
class ListSerializer(Serializer):
    ...
    def read(self, parameterNode, name):
        ...
        ret = [self._elementSerializer.read(parameterNode, paramName(index)) for index in range(self._len(parameterNode, name))]
...
class TupleSerializer(Serializer):
    ...
    def read(self, parameterNode, name: str) -> None:
        return tuple(serializer.read(parameterNode, self._paramName(name, index)) for index, serializer in enumerate(self._serializers))
    ...
    def create(type_):
        if DictSerializer.canSerialize(type_):
            ...
            if len(args) not in (0, 2):
                raise Exception("Unsure how to handle a typed dictionary without exactly 0 or 2 types." " Note that zero types will be dict[typing.Any, typing.Any]")
            if len(args) == 0:
