...
def createGuiConnector(widget, datatype) -> GuiConnector:
    ...
    raise RuntimeError(f"Unable to create GUI connector from datatype '{datatype}' to widget type '{type(widget)}'." " To determine which data types can be connected to which widget types, please check 'canRepresent' methods in 'GuiConnector' classes" f" in these files: {', '.join(guiConnectorFilePaths)}")
    ...
    def __init__(self, widget, type_) -> None:
        ...
        if default is None:
            try:
                ...
                raise RuntimeError("When providing RangeBounds that does not cover the value 0 " + "you also need to provide a Default() value to initialize the GUI")
...
def _extractCorrectWidgets(widget):
    ...
    parentStacks = [_makeParentStack(child, widget) for child in children if child.property(SlicerPackParameterNamePropertyName) is not None]
    ...
    leafParentStacks = [i for i, j in zip(parentStacks[:-1], parentStacks[1:]) if not i == j[: len(i)]] + [parentStacks[-1]]
...
class WidgetChildrenToParameterPackConnector(GuiConnector):
    ...
    def __init__(self, widget: qt.QWidget, datatype):
        ...
        self._nameToConnectorMap = {paramName: createGuiConnector(nameWidgetMap[paramName], self._datatype.dataType(paramName)) for paramName in self._packParamNames}
