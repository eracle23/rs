...
def _eqMethod(self, other) -> bool:
    if type(self) == type(other):
        return all([_readValue(self, parameter.basename) == _readValue(other, parameter.basename) for parameter in self.allParameters.values()])
    else:
        return False
...
def _strMethod(self) -> str:
    strParams = [f"{parameter.basename}={_quoteIfStr(_readValue(self, parameter.basename))}" for parameter in self.allParameters.values()]
...
def createObservedParameterPackImpl(packType):
    class ObservedParameterPack(packType):
        def __init__(self, parameterNode: slicer.vtkMRMLScriptedModuleNode, serializer: Serializer, name: str, args: dict[str, typing.Any]):
        ...
        def __setattr__(self, key, value):
            if self._observedPackValues.frozen and not hasattr(self, key):
                raise AttributeError(f"'ObservedParameterPack({packType.__name__})' has no attribute '{key}'" " and attributes cannot be added dynamcially")
...
class ParameterPackSerializer(Serializer):
    ...
    def create(type_):
        if ParameterPackSerializer.canSerialize(type_):
            return ValidatedSerializer(ParameterPackSerializer(type_), [NotNone(), _ParameterPackInstanceValidator(type_)])
        return None
