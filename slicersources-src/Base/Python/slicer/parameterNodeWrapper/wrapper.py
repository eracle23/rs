...
def _connectParametersToGui(self, mapping):
    ...
    mappingToConnector = {paramName: value if isinstance(value, GuiConnector) else createGuiConnector(value, self.dataType(paramName)) for paramName, value in mapping.items()}
