...
class SlicerApplicationLogHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if hasattr(ctk, "ctkErrorLogLevel"):
            self.pythonToCtkLevelConverter = {logging.DEBUG: ctk.ctkErrorLogLevel.Debug, logging.INFO: ctk.ctkErrorLogLevel.Info, logging.WARNING: ctk.ctkErrorLogLevel.Warning, logging.ERROR: ctk.ctkErrorLogLevel.Error}
        self.origin = "Python"
    ...
    def emit(self, record):
        try:
            ...
            slicer.app.errorLogModel().postEntry(qt.QDateTime.currentDateTime(), threadId, self.pythonToCtkLevelConverter[record.levelno], self.origin, context, msg)
        except:
