...
QPixmap createVisionMagicSplashPixmap()
{
  ...
  // Version info（与 slicer-application-properties.cmake 一致，来自 vtkSlicerVersionConfigure.h）
  ...
  painter.drawText(versionRect, Qt::AlignLeft | Qt::AlignVCenter, QString("V1.0.1.0"));
...
int SlicerAppMain(int argc, char* argv[])
{
  ...
  // DICOM 导入校验：对新加入场景的 Volume 节点进行校验，不通过则提示并移除
  if (vtkMRMLScene* scene = app.mrmlScene())
    {
    static vtkSmartPointer<vtkCallbackCommand> importValidatorCallback;
    if (!importValidatorCallback)
      {
      importValidatorCallback = vtkSmartPointer<vtkCallbackCommand>::New();
      importValidatorCallback->SetCallback(
        [](vtkObject* caller, unsigned long, void*, void* callData)
        {
          vtkMRMLScene* scene = vtkMRMLScene::SafeDownCast(caller);
          vtkMRMLNode* node = static_cast<vtkMRMLNode*>(callData);
          if (!scene || !node)
            {
            return;
            }
          ...
          const QString error = VolumeImportValidator::validate(volumeNode);
          if (!error.isEmpty())
            {
            QMetaObject::invokeMethod(
              ...
              [error]()
              {
                QMessageBox::warning(nullptr, QString::fromUtf8("导入失败"), error);
              });
            scene->RemoveNode(volumeNode);
            }
        });
      }
    scene->AddObserver(vtkMRMLScene::NodeAddedEvent, importValidatorCallback);
    }
