  ...
  for (const UserInfo& user : users)
    {
      ...
      if (user.isLocked)
        ...
        if (user.lockReason == UserLockReason::PasswordFailures)
          {
          status = QString::fromUtf8("已锁定（密码）");
          }
        else if (user.lockReason == UserLockReason::AdminManual)
          {
          status = QString::fromUtf8("已锁定（管理员）");
          }
        else
          {
...
void qUserManagementDialog::onAddUser()
{
  ...
  const QString usernameError = UserManager::validateUsername(username);
  if (!usernameError.isEmpty())
    {
      QMessageBox::warning(this, QString::fromUtf8("输入错误"), usernameError);
      return;
    }
