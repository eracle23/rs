...
//-----------------------------------------------------------------------------
void VolumeImportValidator::installSceneObserver(vtkMRMLScene* scene)
{
  if (!scene)
    {
    return;
  ...
  if (!importValidatorCallback)
    ...
    importValidatorCallback->SetCallback(
      ...
      {
        vtkMRMLScene* s = vtkMRMLScene::SafeDownCast(caller);
        vtkMRMLNode*  node = static_cast<vtkMRMLNode*>(callData);
        if (!s || !node)
          {
          return;
          }
        const QString error = VolumeImportValidator::validate(node);
        if (error.isEmpty())
          {
          return;
          }
        // 校验失败：在 Qt 主线程弹出警告并移除节点
