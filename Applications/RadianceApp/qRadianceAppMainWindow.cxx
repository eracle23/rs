...
QIcon actionIconOverride(QAction* action)
{
  ...
  static const ActionIconRule kActionIconRules[] = {
    {"toolbar_add_data.svg", {
      ...
      u8"添加数据", u8"加载数据", u8"导入场景", u8"加载场景", u8"添加体积", u8"添加变换"
    }},
    ...
    {"toolbar_volumes.svg", {"volumes", "volume", u8"体数据", u8"体积"}}
  };
...
void qRadianceAppMainWindowPrivate::setupUi(QMainWindow * mainWindow)
{
  ...
  mainWindow->menuBar()->insertMenu(this->HelpMenu->menuAction(), userMenu);
...
void qRadianceAppMainWindow::applyShellTweaks()
{
  ...
  if (QStatusBar* sb = this->statusBar())
    {
  ...
  auto hideModuleSearchUI = [this]()
    ...
    if (auto* selector = this->moduleSelector())
      ...
      if (auto* viewMenu = this->findChild<QMenu*>("ViewMenu"))
        {
  ...
  // ========== 布局菜单（视窗布局）白名单 ==========
  // 仅保留以下与 qSlicerMainWindow.ui 中 objectName 一致的布局动作，使视窗按钮可选：
  // - ViewLayoutConventionalAction：常规视窗
  // - ViewLayoutFourUpAction：四格视窗
  // - ViewLayoutOneUp3DAction：单 3D 视窗
  // - ViewLayoutOneUpRedSliceAction / Yellow / Green：单切片视窗
  // 其余布局选项隐藏/禁用。
  const QSet<QString> allowedLayoutActions = {
    "ViewLayoutConventionalAction",        // 常规视窗（UI 中实际名称）
  ...
  QObject::connect(this, &qSlicerMainWindow::initialWindowShown, this, [this, hideModuleSearchUI]() {
    // 延迟再次执行隐藏模块搜索，确保工具栏完全初始化后生效
    QTimer::singleShot(100, this, hideModuleSearchUI);
    ...
    // ========== 模块下拉菜单过滤：精简为第二张图样式（添加DICOM数据、体积、分割编辑器、标记、模型 + 分类子菜单）==========
