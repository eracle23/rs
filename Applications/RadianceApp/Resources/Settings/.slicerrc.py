...
def hideUnwantedUIElements():
    ...
    try:
        for n in (
            "HelpReportBugOrFeatureRequestAction",
            "HelpSearchFeatureRequestsAction",
            "HelpDocumentationAction",
            "HelpBrowseTutorialsAction",
            "HelpAcknowledgmentsAction",
            "HelpAboutSlicerAppAction",
        ):
            _hideActionByName(mainWindow, n)
    except Exception as e:
        _appendMarkerLog("hideByHelpNames error: %s" % str(e))
    try:
        _hideActionsContainingText(
            mainWindow,
            [
                "Documentation",
                "Tutorial",
